...
export default function InterviewPage() {
  ...
  const submitAnswer = async (answerText) => {
    if (!answerText.trim() || loading || phase === 'done') return;
    const timeSpent = Math.round((Date.now() - qStartTime) / 1000);
    setInput('');
    push('user', answerText.trim());
    setLoading(true);
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          {phase === 'questioning' && (
            <TimerRing key={timerKey} seconds={timerSecs} total={timerSecs}
              onExpire={() => input.trim() && submitAnswer(input)}/>
          )}
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:10, alignItems:'flex-end' }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder={phase === 'loading' ? 'Starting interview…' : phase === 'done' ? 'Interview complete' : 'Type your answer… (Enter to send, Shift+Enter for new line)'}
            rows={2}
            style={{
